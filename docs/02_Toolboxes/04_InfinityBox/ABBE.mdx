...
Let's talk about the question: *Why does a microscope not resolve better than half the (detection) wavelength? And why Abbe knew it already >150 years ago. And yet, why Jena?!
...
\\\text{With}\\ (f_{\text{obj}}=50,\text{mm}) \\and\\ (f_{\text{tube}}=100,\text{mm}):
[
M=-2
]
$$
...
\\(\text{in air} (n\approx 1)). \text{For a lens of diameter (d) and focal length (f)}:\\
[
