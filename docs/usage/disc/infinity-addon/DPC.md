...
dpc_images = np.array(
    [
        io.imread(data_path + image_list[image_index])
...
```python
dpc_solver_obj = DPCSolver(
    dpc_images, wavelength, na, na_in, pixel_size, rotation, dpc_num=dpc_num
)
```
...
# plot the sources
max_na_x = max(
    dpc_solver_obj.fxlin.real * dpc_solver_obj.wavelength / dpc_solver_obj.na
)
min_na_x = min(
    dpc_solver_obj.fxlin.real * dpc_solver_obj.wavelength / dpc_solver_obj.na
)
max_na_y = max(
    dpc_solver_obj.fylin.real * dpc_solver_obj.wavelength / dpc_solver_obj.na
)
min_na_y = min(
    dpc_solver_obj.fylin.real * dpc_solver_obj.wavelength / dpc_solver_obj.na
)
...
for plot_index, source in enumerate(list(dpc_solver_obj.source)):
    ...
    plot_col = np.mod(plot_index, 2)
    ax[plot_row, plot_col].imshow(
        np.fft.fftshift(dpc_solver_obj.source[plot_index]),
        cmap="gray",
        clim=(0, 1),
...
for plot_index in range(ax.size):
    ...
    if plot_row == 0:
        plot = ax[plot_row, plot_col].imshow(
            np.fft.fftshift(dpc_solver_obj.Hu[plot_col].real),
            cmap="jet",
            extent=[min_na_x, max_na_x, min_na_y, max_na_y],
            clim=[-2.0, 2.0],
        )
        ax[plot_row, plot_col].set_title("Absorption WOTF {:02d}".format(plot_col))
        plt.colorbar(plot, cax=cax, ticks=[-2.0, 0, 2.0])
    else:
        plot = ax[plot_row, plot_col].imshow(
            np.fft.fftshift(dpc_solver_obj.Hp[plot_col].imag),
            cmap="jet",
            extent=[min_na_x, max_na_x, min_na_y, max_na_y],
            clim=[-0.8, 0.8],
        )
        ax[plot_row, plot_col].set_title("Phase WOTF {:02d}".format(plot_col))
        plt.colorbar(plot, cax=cax, ticks=[-0.8, 0, 0.8])
...
plot = axes[0].imshow(
    ...
    clim=[-0.15, 0.02],
    cmap="gray",
    extent=[0, dpc_result[0].shape[-1], 0, dpc_result[0].shape[-2]],
)
axes[0].axis("off")
